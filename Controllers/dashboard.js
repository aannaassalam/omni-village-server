        ...
        const cultivation_data = await Cultivation.aggregate([
            ...
            // },
            {
                $project: {
                    ...
                    fertilizer_used:
                        "$important_information.type_of_fertilizer_used",
                    pesticide_used:
                        "$important_information.type_of_pesticide_used",
        ...
        const fishery_data = await Fishery.aggregate([
            ...
            // {
            {
                $project: {
                    ...
                    sold_to_neighbour:
                        "$production_information.sold_to_neighbours",
                    sold_to_market:
                        "$production_information.sold_for_industrial_use",
                    ...
                    expenditure:
                        "$production_information.expenditure_on_inputs",
                    currency: "$user.currency",
                    type: "Fishery",
                },
            },
        ...
        const poultry_data = await Poultry.aggregate([
            ...
            },
            {
                $group: {
                    ...
                    sold_to_market: {
                        $sum: "$products.sold_for_industrial_use",
                    },
                ...
                current[1].forEach((_item) => {
                    ...
                    if (_item.fed_to_livestock) {
                        obj.fed_to_livestock =
                            (obj.fed_to_livestock || 0) +
                            _item.fed_to_livestock;
                    }

                    obj.output = (obj.output || 0) + _item.output;
                    obj.self_consumed =
                        (obj.self_consumed || 0) + _item.self_consumed;
                    obj.sold_to_neighbour =
                        (obj.sold_to_neighbour || 0) + _item.sold_to_neighbour;
                    obj.sold_to_market =
                        (obj.sold_to_market || 0) + _item.sold_to_market;
                    obj.wastage = (obj.wastage || 0) + _item.wastage;
                    obj.other_value =
                        (obj.other_value || 0) + _item.other_value;
                    obj.income =
                        (obj.income || 0) +
                        fx.convert(_item.income, {
                            from: _item.currency,
                            to: "USD",
                        });
                    obj.expenditure =
                        (obj.expenditure || 0) +
                        fx.convert(_item.expenditure, {
                            from: _item.currency,
                            to: "USD",
                        });
                });
...
module.exports.bifurcated_chart_crop = async (req, res) => {
    ...
    try {
        ...
        if (sorted_yeild_arr.length % 2 === 0) {
            median =
                (sorted_yeild_arr[even_div - 1] +
                    sorted_yeild_arr[odd_div - 1]) /
                2;
        } else {
    ...
    try {
        ...
        const income_expenditure_data = Object.entries(grouped_arr).reduce(
            (prev, current) => {
                ...
                return [
                    ...prev,
                    { name: current[0], ...income_expenditure_obj },
                ];
            },
            []
        );
...
module.exports.storage_data = async (req, res) => {
    ...
    grouped_data.forEach((_data) => {
        if (_data.stock_name.includes("grain")) {
            aggregated_data.grain[_data.storage_method_name] = aggregated_data
                .grain[_data.storage_method_name]
            ...
            aggregated_data.meat[_data.storage_method_name] = aggregated_data
                .meat[_data.storage_method_name]
    ...
    try {
        ...
        tree.forEach((_tree) => {
            tree_crop_count[_tree.crop.name.en] =
                (tree_crop_count[_tree.crop.name.en] || 0) +
                _tree.number_of_trees;
        ...
        const fish_from_pond = await Fishery.aggregate([
            ...
            },
            {
                $group: {
                    ...
                    count: { $sum: "$important_information.number_of_fishes" },
                    feed_quantity: {
                        $sum: "$production_information.total_feed",
                    },
                },
            },
            {
    ...
    try {
        ...
        const structured_data = consumption_docs.reduce((prev, current) => {
            ...
            prev.purchased_from_neighbours =
                (prev.purchased_from_neighbours || 0) +
    ...
    try {
        ...
        const new_consumption_docs = consumption_docs.map((_consumption) => {
            ...
            if (_consumption.consumption_crop._id) return _consumption;
                ...
                _consumption.consumption_crop =
                    _consumption.tree_consumption_crop;
            } else if (_consumption.poultry_consumption_crop._id) {
                _consumption.consumption_crop =
                    _consumption.poultry_consumption_crop;
            } else if (_consumption.hunting_consumption_crop._id) {
                _consumption.consumption_crop =
                    _consumption.hunting_consumption_crop;
            } else if (_consumption.fishery_consumption_crop._id) {
                _consumption.consumption_crop =
                    _consumption.fishery_consumption_crop;
            } else if (_consumption.other_consumption_crop._id) {
                _consumption.consumption_crop =
                    _consumption.other_consumption_crop;
            }
            return _consumption;
        });
        ...
        const processed_data = Object.entries(grouped_by_crop_data).map(
            (_item) => {
...
module.exports.consumption_by_crop = async (req, res) => {
    ...
    try {
        ...
        new_consumption_docs.forEach((_consumption) => {
            ...
            final_obj.purchased_from_market +=
                _consumption.purchased_from_market;
...
module.exports.ideal_consumption_by_label = async (req, res) => {
    ...
    try {
        ...
        const new_consumption_docs = consumption_aggregated.map(
            (_consumption) => {
                _consumption.output = weightConverter(
                    _consumption.total_quantity,
                    _consumption.weight_measurement
        ...
        const processed_data = Object.entries(grouped_by_label).map((_item) => {
            ...
            const total_ideal_consumption = _item[1].reduce(
                (prev, current) =>
                    (prev +=
                        current.consumption_crop.ideal_consumption_per_person),
                0
            );
...
module.exports.deficiet_chart = async (req, res) => {
    let { village } = req.query;
    village = typeof village === "string" ? [village] : village ?? "rukha";
        ...
        const type_based_cultivation = await Cultivation.aggregate([
            ...
            // },
            {
                $project: {
                    ...
                    ideal_consumption: {
                        $multiply: [
                            "$crop.ideal_consumption_per_person",
                            "$count",
                        ],
                    },
                },
        ...
        const type_based_fishery = await Fishery.aggregate([
            ...
            },
            {
                $group: {
                    ...
                    doc: { $mergeObjects: "$$ROOT" },
                    self_consumed: {
                        $sum: "$production_information.self_consumed",
                    },
                    count: { $sum: 1 },
                },
            },
        ...
        new_arr.forEach((_item) => {
            processed_cultivation_arr[_item.label] = {
                self_consumed:
                    (processed_cultivation_arr[_item.label]?.[
                        "self_consumed"
                    ] || 0) + _item.self_consumed,
                ideal_consumption:
                    (processed_cultivation_arr[_item.label]?.[
                        "ideal_consumption"
                    ] || 0) + _item.ideal_consumption,
            };
        });

        console.log(processed_cultivation_arr);
...
module.exports.food_balance = async (req, res) => {
    ...
    try {
        ...
        const type_based_fishery = await Fishery.aggregate([
            ...
            },
            {
                $group: {
                    ...
                    output: {
                        $sum: "$production_information.production_output",
                    },
        ...
        const processed_cultivation_data = type_based_cultivation.map(
            (_item) => {
